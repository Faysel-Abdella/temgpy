...
export default function Home({ project }: { project: Project }) {
  useEffect(() => {
    import("aos").then((AOS) => {
      Aos.init();
    });
  }, []);
  const images = [project.banners[0], project.banners[1], project.banners[2]];
  ...
  return (
    <section className="flex bg-background2 relative text-background2-foreground flex-col py-10">
      ...
      <div className="w-full h-fit  flex flex-col items-center justify-center gap-8">
        <div className="flex px-4 flex-col items-center justify-center gap-6">
          <p
            ...
            className="text-background2-description font-sans text-md  text-center font-normal">
